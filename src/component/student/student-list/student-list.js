...
export function StudentList() {
  ...
  function onUpdateStudent(student) {
    updateStudent(student).then(
        (updatedStudent) => {
          const index = students.findIndex(
              (currentStudent) => updatedStudent.id === currentStudent.id);
          const newStudentArray = [...students];
  ...
  return (
    <div className="student-list-container">
      <div className="student-button-section">
        ...
        <button className="themed-button bg-secondary button-padding"
          onClick={() => sortBy('lastname')}>Trier par nom</button>

        <button className="themed-button bg-alternative button-padding"
          onClick={() => sortBy('firstname')}>Trier par prénom</button>
      ...
      <ul className="student-list ">
        ...
        {students?.map((student) =>
          <Popover
            ...
            positions={['right', 'top']}
            content={<img className="img-popover"
              src={getImgUrl(student.img)} alt="Photographie de l'élève" />}
          >
            <li
              key={student.id}
